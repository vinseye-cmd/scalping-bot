...
def should_heartbeat(state: dict) -> bool:
    ...
    return elapsed >= timedelta(minutes=HEARTBEAT_INTERVAL_MIN)
...
def run():
    ...
    try:
        ...
        if should_heartbeat(state):
            ...
            if sig_hb:
                fib_str = (f"Fib 0.5 zone : `{round((sl_hb + tp_hb) / 2, 2):,.2f}`\n"
                           f"SL (Fib 1) : `{sl_hb:,.2f}` | TP (Fib 0) : `{tp_hb:,.2f}`")
            else:
                fib_str = "Aucun niveau Fib 0.5 identifie actuellement"

            pos_str = (
                f"Position ouverte : `{state['position']['side'].upper()}` @ `{state['position']['entry']:,.2f}`"
                if state.get("position") else "Pas de position ouverte"
            )
            lock_str = ""
            if is_locked(state):
                lu = datetime.fromisoformat(state["locked_until"]).astimezone(PARIS_TZ)
                lock_str = f"\nPause active jusqu'a : `{lu.strftime('%H:%M')}`"
        ...
        if state["position"]:
            ...
            if not position_exists:
                ...
                if net < 0:
                    ...
                    if state["consecutive_losses"] >= config["max_losses"]:
                        ...
                        lu_paris = datetime.fromisoformat(locked_until).astimezone(PARIS_TZ)
                        tg(config["tg_token"], config["chat_id"],
                           f"*PAUSE ACTIVEE — {state['consecutive_losses']} pertes consecutives*\n"
                           f"Reprise automatique a `{lu_paris.strftime('%H:%M')}` (Paris)")
                else:
                ...
                state["position"] = None
                save_state(state)
                print(f"[{now_str}] Position cloturee MoonX. Net: {net:+.4f}")
                tg(config["tg_token"], config["chat_id"],
            ...
            if not pos["tp1_hit"]:
                ...
                if reversed_:
                    ...
                    state["position"] = None
                    save_state(state)
                    print(f"[{now_str}] Sortie SuperTrend @ {price_now:.2f} | PnL: {pnl_pct:+.2f}%")
                    tg(config["tg_token"], config["chat_id"],
                ...
                print(f"[{now_str}] En attente TP2={pos['tp2']:.2f} | SL trailing={pos['sl']:.2f} | PnL: {pnl_pct:+.2f}%")
        ...
        # ── 2. RECHERCHE DE SIGNAL (24h/24) ─────────────────────────
            ...
            # Notification changement de tendance HTF
            if htf_trend != state.get("last_htf_trend", 0):
                ...
                tg(config["tg_token"], config["chat_id"],
                   ...
                   f"BTC : `{price:,.2f}` USDT\n"
                   f"Le bot cherche des entrees *{htf_label}S*")
                ...
                if signal != state.get("last_signal"):
                    ...
                    if (signal == "LONG" and htf_trend != 1) or (signal == "SHORT" and htf_trend != -1):
                        ...
                        tg(config["tg_token"], config["chat_id"],
                           ...
                           f"Raison : Tendance 1h {htf_label} (contre-tendance)")
                        return
